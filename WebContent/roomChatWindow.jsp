	...
	<script language="javascript" type="text/javascript">
		...
		function onOpen(evt)
		{
			...
			$.post("/GCM-App-Server/AuthServlet",
	          		  { checkReq: "getAllRoomMessagesWebApp",roomName: $('#roomName').val()},
	          		  function(data){
	          			  var obj = jQuery.parseJSON(data);
						$.each(obj,function(){
							/*if(this.roomUsername == $('#name').val()){
								writeToScreen($('#name').val()+": "+this.message);
							}else{
								writeToScreen($('#toEmail').val()+": "+this.message);
							}*/
							writeToScreen(this.roomUsername+": "+this.message);
						});
	          		  }
        		);
		...
		function doSend(message, from, to)
		{
		        ...
		        $.post("/GCM-App-Server/AuthServlet",
		          		  { checkReq: "getAllRoomMembersWebApp",roomName: $('#roomName').val()},
		          		  function(data){
		          			  var obj = jQuery.parseJSON(data);
							$.each(obj,function(){
								var strucMessage = message + '<:>' + from + '<:>' + this.email;
		        ...
		        if ( $('#value').val().length > 0 ){
		        	//Send to db
		        	$.post("/GCM-App-Server/AuthServlet",
			          		  { checkReq: "sendMessageToRoomWebApp",messageToRoom: $('#value').val(), username: $('#name').val(), roomName: $('#roomName').val() },
			          		  function(data){
		        	...
		        	$.post("/GCM-App-Server/GCMNotification",
			          		  { fromWho: "computer", messageType: "room", roomMessage: $('#value').val(), fromEmail: $('#name').val(), roomName: $('#roomName').val() },
			          		  function(data){
        ...
        <div class="row">
            ...
            <div class="col-md-9">
		        ...
		        <div>
		            ...
		            <input type="hidden" maxlength="" name="roomName" id="roomName" value="<%=request.getParameter("roomName") %>"/>
		            <input type="button" value="Send" onclick="doSend(document.getElementById('value').value,document.getElementById('name').value,document.getElementById('roomName').value)" />
		        </div>
